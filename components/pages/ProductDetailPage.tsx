...
const ProductDetailPage = ({ currentProduct }: { currentProduct: any }) => {
  ...
  const primaryImage = useMemo(() => {
    ...
    return (
      ...
      resolveProductImage(currentProduct)
    );
  }, [currentProduct]);
  ...
  const galleryImages = useMemo(() => {
    if (!currentProduct?.gallery || currentProduct.gallery.length === 0)
      return [resolveProductImage(currentProduct)];
    return currentProduct.gallery.map((img: any) => img.url).filter(Boolean);
  }, [currentProduct]);
  ...
  const currentPrice = useMemo(() => {
    const price = selectedVariant
      ? resolveCommercePrice(selectedVariant)
      : resolveCommercePrice(currentProduct || {});
    return formatCommercePrice(price, currentProduct?.currency || "INR");
  }, [selectedVariant, currentProduct]);

  // Get compare at price
  const compareAtPrice = useMemo(() => {
    const price = resolveCompareAtPrice(currentProduct || {});
    return price === null
      ? null
      : formatCommercePrice(price, currentProduct?.currency || "INR");
  }, [currentProduct]);

  // Calculate discount percentage
  const discountPercentage = useMemo(() => {
    const original = resolveCompareAtPrice(currentProduct || {});
    const current = resolveCommercePrice(currentProduct || {});
  ...
  const stockStatus = useMemo(() => {
    ...
    return currentProduct?.status === "active"
      ? { text: "Available to order", color: "text-green-500" }
      : { text: "Unavailable", color: "text-red-500" };
  }, [selectedVariant, currentProduct?.status]);
    ...
    <div className="mx-auto px-[5%] pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_480px] gap-12 items-start">
        ...
        <div className="lg:sticky lg:top-[128px] space-y-8">
          ...
          {/* Hurry up */}
          {leftInStock !== null && leftInStock > 0 && leftInStock < 5 && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              <div className="space-y-6">
                ...
                {Array.isArray(currentProduct.features) &&
                  ...
                  <ul className="space-y-4">
                    {currentProduct.features.map((feature: string) => (
                    <li
                      key={feature}
                      className="flex items-center gap-3 font-bold text-foreground/80"
                    >
